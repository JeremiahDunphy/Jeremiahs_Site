  ...
  ({testimonial: {text, name, image}, isActive}) => (
    <div
      ...
      <div className="flex flex-col gap-y-4">
        <p className="prose prose-sm font-medium italic text-black dark:text-white sm:prose-base">{text}</p>
        <p className="text-xs italic text-black dark:text-white sm:text-sm md:text-base lg:text-lg">-- {name}</p>
      </div>
    </div>
  ),
);
